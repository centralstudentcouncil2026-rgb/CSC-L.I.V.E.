...
create table if not exists public.attendance_settings (
    ...
    session_title text not null default 'Morning Session',
    point_settings jsonb not null default '{
        "parameters": {
            ...
            "dean_points": 50,
            "student_from_college_points": 0,
            "faculty_points": 5,
            "sponsor_points": 5,
            "student_player_points": 0,
...
alter table public.attendance_settings
    ...
    add column if not exists session_title text not null default 'Morning Session',
...
alter table public.attendance_settings
    alter column session_title set default 'Morning Session';
...
alter table public.attendance
    ...
    add column if not exists attendance_session_title text not null default 'Morning Session',
    add column if not exists attendance_session_id bigint,
...
alter table public.attendance
    alter column attendance_session_title set default 'Morning Session';
...
create table if not exists public.attendance_sessions (
    ...
    updated_at timestamptz not null default now(),
    unique (category, attendance_date)
);
...
with ranked_attendance_duplicates as (
    select
        ctid,
        row_number() over (
            partition by student_id, attendance_date, attendance_session_title
...
drop index if exists public.attendance_student_date_unique_idx;
drop index if exists public.attendance_student_date_session_unique_idx;

create unique index attendance_student_date_session_unique_idx
on public.attendance (student_id, attendance_date, attendance_session_title);

insert into public.attendance_sessions (category, attendance_date)
select distinct attendance_session_title, attendance_date
from public.attendance
where attendance_session_title is not null
  and attendance_date is not null
on conflict (category, attendance_date) do nothing;

update public.attendance a
set attendance_session_id = s.id
from public.attendance_sessions s
where s.category = a.attendance_session_title
...
set session_title = 'Morning Session'
where session_title in ('Regular Day', 'Regular Attendance');

update public.attendance
set attendance_session_title = 'Morning Session'
where attendance_session_title in ('Regular Day', 'Regular Attendance');
...
set point_settings = jsonb_set(
    ...
    jsonb_build_object(
        ...
        'student_from_college_points', coalesce(point_settings #> '{parameters,student_player_points}', point_settings #> '{parameters,student_from_college_points}', point_settings #> '{opening_program,student_from_college_points}', '0'::jsonb),
        ...
        'student_player_points', coalesce(point_settings #> '{parameters,student_player_points}', point_settings #> '{parameters,student_from_college_points}', point_settings #> '{sunday_devotional,student_player_points}', '0'::jsonb),
...
set attendance_point_settings = jsonb_set(
    ...
    jsonb_build_object(
        ...
        'student_from_college_points', coalesce(attendance_point_settings #> '{parameters,student_player_points}', attendance_point_settings #> '{parameters,student_from_college_points}', attendance_point_settings #> '{opening_program,student_from_college_points}', '0'::jsonb),
        ...
        'student_player_points', coalesce(attendance_point_settings #> '{parameters,student_player_points}', attendance_point_settings #> '{parameters,student_from_college_points}', attendance_point_settings #> '{sunday_devotional,student_player_points}', '0'::jsonb),
...
drop policy if exists "Dashboard users can read attendance sessions" on public.attendance_sessions;
create policy "Dashboard users can read attendance sessions"
...
safe_parameters as (
    select
        ...
        case when coalesce(point_settings #>> '{parameters,student_player_points}', '') ~ '^\d+(\.\d+)?$'
             ...
             then (point_settings #>> '{parameters,student_player_points}')::numeric else 0 end as student_player_points,
...
attendance_counts as (
    select
        ...
        count(*) filter (
            where lower(coalesce(na.status, 'present')) = 'present'
              and coalesce(na.attendance_category, '') not in ('faculty', 'department_chair', 'dean', 'sponsor', 'sponsor_game_attendance')
